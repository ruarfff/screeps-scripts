...
import RuarfffCreep from "./RuarfffCreep";
...
export const loop = (): void => {
  ...
  if (HOME.spawning) {
    const spawningCreep: RuarfffCreep = Game.creeps[HOME.spawning.name];
  ...
  for (const name in Game.creeps) {
    const creep: RuarfffCreep = Game.creeps[name];
...
function spawnCreepsByRole(
  spawnPoint: StructureSpawn,
  role: string,
  amount = 2
) {
  const creeps = _.filter(
    Game.creeps,
    (creep: { memory: { role: string } }) => creep.memory.role == role
  );
